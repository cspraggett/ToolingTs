...
export function ResultDisplay({ result, labels }: ResultDisplayProps) {
  ...
  return (
    <ul className="grid gap-2 grid-cols-1 sm:grid-cols-2">
      {summary.map((item) => (
        <li key={item.size} className="flex gap-4 items-center bg-slate-50/50 border-2 border-slate-100 rounded-xl px-4 py-3 shadow-sm hover:border-primary/20 transition-colors">
